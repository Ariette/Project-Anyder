    ...
    public class SearchOutput
    {
        ...
        void WriteIndex()
        {
            ...
            foreach (var item in _db.Items)
                WriteIndex(item, "item", (string)item.en.name);
            ...
            foreach (var spot in _db.FishingSpots)
                WriteIndex(spot, "fishing", (string)spot.en.name);

            // Mobs
            foreach (var mob in _db.Mobs)
                WriteIndex(mob, "mob", (string)mob.en.name);
            ...
            foreach (var action in _db.Actions)
                WriteIndex(action, "action", (string)action.en.name);

            // Leves
            foreach (var leve in _db.Leves)
                WriteIndex(leve, "leve", (string)leve.en.name);

            // Quests
            foreach (var quest in _db.Quests)
                WriteIndex(quest, "quest", (string)quest.en.name);

            // Achievements
            foreach (var achievement in _db.Achievements)
                WriteIndex(achievement, "achievement", (string)achievement.en.name);

            // Instances
            foreach (var instance in _db.Instances)
                WriteIndex(instance, "instance", (string)instance.en.name);

            // Fates
            foreach (var fate in _db.Fates)
                WriteIndex(fate, "fate", (string)fate.en.name);

            // Statuses
            foreach (var status in _db.Statuses)
                WriteIndex(status, "status", (string)status.en.name);
        }
        ...
        void WriteIndex(dynamic obj, string type, string key_en)
        {
